...
| CI workflows | GitHub push/PR | Source -> CI | CodeQL, govulncheck, test, and lint run; actions use moving version tags. | `.github/workflows/*.yml` |
...
| TM-005 | CI supply-chain attacker | Ability to alter a referenced action/dependency or trusted source change. | Inject code during CI/build. | Runtime controls can be removed before deployment. | Source/build integrity. | PR CI runs tests, lint, CodeQL, and govulncheck (`.github/workflows`). | Actions are tag-pinned, not commit-SHA pinned; no provenance/release policy is documented. | Pin actions to reviewed SHAs; protect `main`; require code-owner review; publish signed artifacts and SBOM. | Monitor workflow/action revision changes and dependency alerts. | Low | High | medium |
