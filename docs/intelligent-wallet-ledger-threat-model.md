...
| TM-005 | CI supply-chain attacker | Ability to alter a referenced action/dependency or trusted source change. | Inject code during CI/build. | Runtime controls can be removed before deployment. | Source/build integrity. | PR CI runs tests, lint, Helm, k6, CodeQL, and govulncheck; third-party actions are commit-SHA pinned (`.github/workflows`). | No provenance/release policy, SBOM, branch protection, or required code-owner review is evidenced in this repository. | Protect `main`; require code-owner review; publish signed artifacts and SBOM; review and periodically update pinned action SHAs. | Monitor workflow/action revision changes and dependency alerts. | Low | High | medium |
