...
| Kafka consumers | configured broker/topic | Kafka -> workers | Known foreign events are ignored; invalid or unknown events are durably quarantined before offset commit. | ADR-0007, `internal/quarantine/postgres` |
...
| TM-003 | Broker peer or compromised integration | Kafka access or weak broker network isolation. | Send malformed, unknown-version, or high-volume event. | Consumer lag; delayed review or posting. | Workflow availability, event integrity. | Versioned envelopes, inbox dedupe, transactional PostgreSQL effects, and idempotent bounded PostgreSQL quarantine before offset commit. | No broker TLS/SASL/ACL, quarantine retention policy, automated replay, or deployment monitoring is implemented here. | Use TLS/SASL and topic ACLs; alert on quarantine growth; define retention and event-contract ownership. | Alert on quarantine growth, parser errors, and offset lag. | Medium | Medium | medium |
